...
//    const allItem = items.data.items
    ...
    //    console.log("HELLO ",objItems) /*  */
      ...
      var dataRange = '{"values": [['+ "'" + JSON.stringify(objItems)  + "'" + ',"' + myreceiveemail + '"' +  ',"' + ProjectNumber + '"' + ',"' + Region + '"' + ',"' + Studio + '"'  + ',"' + ProjectName + '"' + ',"' + strDate + '"' + ',"'  + RecordID + '"' + ',"' + RecordStatus + '"' + ',"' + Symbol + '"'  + ']],"majorDimension": "ROWS"}'
        var configUpdate = {
            method: 'put',
            url: `https://sheets.googleapis.com/v4/spreadsheets/1wH4JcFpnmg1ULmjzOSqiev0c-aD-c_ghDQvpFNkDAsQ/values/Sheet1!A${rowposition}:J${rowposition}?valueInputOption=USER_ENTERED`,
            headers: {
    ...
    //            console.log(error) 
     ...
     React.useEffect(()=>{
        fetch("https://docs.google.com/spreadsheets/d/1wH4JcFpnmg1ULmjzOSqiev0c-aD-c_ghDQvpFNkDAsQ/gviz/tq?tqx=out:json&tq&gid=0")
        .then(resp => resp.text())
        .then(data => {
            const temp1 = data.substring(47).slice(0,-2);
            const json1 = JSON.parse(temp1);
        //    console.log('GET DATA FROM - WFM,Hubspot,Google Data link to Integrate Tab- use tabname -Integrate ',json.table.rows);
        ...
        //                    console.log("LOOK AT DATA ...",JSON.parse(row.c[0].v))
                        ...
                        //    const totalscore = accu1 + accu2 + accu3 + accu4 + accu5 + accu6 + accu7 + accu8 
                            const totalscore = accu1 + accu2 + accu3 + accu4 + accu5 + accu6 + accu7  
